...
- [x] qa-loading-states — For EVERY async operation, specify: skeleton shape (table skeleton, card skeleton, form skeleton), spinner placement, pending text. Include: page load, form submit, inline edit save, filter change, pagination. [DONE: 10 page-load skeletons (DashboardSkeleton, IntakeSkeleton, ProcurementSkeleton, DeploymentWizardSkeleton, FinancialsWizardSkeleton, InventorySkeleton, FinancialsDashboardSkeleton, SettingsSkeleton, ProjectShellSkeleton, plus full-page LoadingSpinner for auth/callback and new-project). 35 form-submit operations catalogued with idle→pending label pairs. Deployment checklist optimistic toggle pattern (no spinner, rollback on error). Note auto-save inline indicator (Saving.../Saved/Failed). Stage-advance operations (wizard overlay opacity-50 + LoadingSpinner). Button pending pattern: InlineSpinner + disabled. TanStack Router pendingMs=200 + pendingMinMs=300 global config. Summary table of all 28 async operation categories.]
- [x] qa-toast-messages — For EVERY mutating operation (create/update/delete), specify: success toast text, error toast text, toast duration (3s default), toast position (bottom-right). Example: "Project created" (success), "Failed to create project" (error), "Invoice marked as paid" (success). [DONE: 65 toast operations across 11 sections. Auth (1), Dashboard (2), Intake (3), Procurement: BOM/Inventory/PO/Packing/CC/Signs/Advance (17), Deployment: Checklist/Token/Phase-panels/Status-advance (13), Financials: Invoicing/Expenses/Go-Live (12), Inventory: Adjust/Threshold/PO/Reconciliation (8), Settings: Pricing/Catalog/Team/Travel (12), Guard/Navigation warnings (2). Sonner Toaster config specified with duration rules (success=3s default, error=5s, warning=4s). Optimistic update error pattern documented. Silent-on-success ops enumerated. All dynamic text interpolations specified.]
- [x] qa-keyboard-nav — For wizard forms: specify Tab order, Enter key behavior (submit step vs submit form), Escape key behavior (close modal, cancel edit). For tables: specify if rows are focusable, Enter to open. [DONE: Full keyboard nav spec in final-mega-spec/ui-spec/keyboard-nav.md. Tab order for all 6 intake steps, procurement tabs, deployment checklist, financials tabs. Enter: submits form on text/number inputs, newline on textarea, toggles checkbox, opens Select/Combobox. Escape: closes any open Dialog/Select/DatePicker/Combobox, collapses inline note panel. Dashboard table rows: tabIndex=0, Enter navigates to project, Arrow Up/Down moves between rows. Inventory/P&L/settings tables: NOT row-focusable (action buttons per row instead). All 12 dialog first-focus targets specified (destructive dialogs focus Cancel by default). Focus management: first-field after step advance, trigger-return after dialog close, next-row after delete. aria-disabled pattern for conditionally disabled buttons (keeps in Tab order for tooltip). 19 ARIA live regions. keyboard-nav.ts utility module spec'd with focusFirst/focusMainContent/handleRowKeyDown helpers.]
