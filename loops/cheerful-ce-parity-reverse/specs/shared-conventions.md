...
Slack event arrives
    ...
    │   - Calls get_slack_user_mapping(deploy_environment)
    │     which reads SLACK_USER_MAPPING dict in constants.py
    │     keyed by deploy environment ("production" or "staging")
    │   - Maps: Slack user ID → Cheerful user UUID (e.g. "83a3177e-...")
    │   - Constructs: RequestContext(
    │       slack_user_id="U099FQLC9PG",
    │       cheerful_user_id="83a3177e-0307-4e5f-ae4e-4bc823db56e9"
    │     )
    │
    ▼ ToolRegistry.call_tool()
    │   - Passes request_context as positional argument to every tool handler
    │
    ▼ Tool handler (e.g. cheerful_list_campaigns)
    │   - Calls _resolve_user_id(request_context)
    │   - If request_context is None OR cheerful_user_id is empty string:
...
- All `/api/service/*` routes require this header
