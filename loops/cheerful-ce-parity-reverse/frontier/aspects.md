...
- [x] **w1-search** — Extract all search/discovery capabilities: 13 capabilities across 7 sub-domains identified. 3 existing CE tools (search_emails, find_similar_emails, search_campaign_creators — all read-only). 8-9 new tools needed. Sub-domains: full-text email search (covered), semantic pgvector search (covered), cross-campaign creator search (covered), IC similar/keyword creator discovery (2 endpoints, no service routes), creator enrichment + profile (2 endpoints, no service routes), YouTube lookalike search (1 endpoint, no service route), lookalike suggestion management (4 webapp-only Next.js routes, no backend endpoints). Key discoveries: lookalike suggestions are webapp-only (Supabase direct, no backend API), existing service endpoints have security gaps (no user_id ownership validation on thread search and RAG similar), auto-discovery config is campaign-level (no dedicated endpoints), YouTube enrichment maps fields differently than Instagram. Sources verified: `creator_search.py` (533 lines), `youtube.py` (236 lines), `service.py` (360 lines), `rag.py` (132 lines), `embedding.py` (32 lines), `email_reply_example.py` (183 lines), `influencer_club.py` models (207 lines), `campaign_lookalike_suggestion.py` (DB model + repository), webapp lookalike suggestion routes (4 files), webapp search hooks/stores
