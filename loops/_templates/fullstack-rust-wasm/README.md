...
├── README.md                              # This file
├── reverse/
│   ├── PROMPT.md.template                 # 6-wave reverse loop: domain → engine → bridge → frontend → UI → synthesis
│   └── frontier/
│       └── aspects.md.template            # Pre-structured frontier with all 6 waves
└── forward/
    ├── PROMPT.md.template                 # 5-phase forward loop: engine → WASM → frontend foundation → components → polish
...
This prevents the "mock gap" — where the frontend is built against a mock that doesn't match the real engine's strictness requirements.
