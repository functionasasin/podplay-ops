...
| Element | Description |
|---------|-------------|
| Logo | TaxKlaro wordmark logo at top-left. Replaced by white-label logo if `white_label_logo_url` is set. Image: max height 40px, width auto-scaled to maintain aspect ratio. |
...
| Center | "Generated by TaxKlaro on {date} at {time} PHT" in 8pt gray. Date format: "March 2, 2026". If white-label: "Powered by TaxKlaro" in 8pt gray. |
...
If an election was made (not optimizer mode): "NOTE: You have elected Path {X}. TaxKlaro recommends Path {Y} which would save ₱{savings}." in an amber advisory box.
...
**Note:** "Source: BIR Form 2307 (Certificate of Creditable Tax Withheld at Source). Verify all entries against original Form 2307 certificates. Figures shown here are as entered by the taxpayer and have not been verified by TaxKlaro."
...
| 4 | Taxpayer Identification Number (TIN) | "{Leave blank — enter your TIN}" | Not stored by TaxKlaro |
...
- The TaxKlaro wordmark in the header is replaced by the provided logo.
...
- The footer center text changes from "Generated by TaxKlaro on {date}" to "Prepared for {client display name or 'Client'} on {date} — Powered by TaxKlaro".
- "Powered by TaxKlaro" is ALWAYS present in the footer (required by license). Font size: 7pt, color: #A0AEC0 (gray-400).
...
**Bucket name:** `taxklaro-pdf-exports` (production); `taxklaro-pdf-exports-staging` (staging).
...
```json
{
  ...
  "results_url": "https://api.taxklaro.ph/v1/batch/batch_uuid_here/results"
}
```

**Webhook registration:** Via `POST /webhooks` (see [api/endpoints.md §14](../api/endpoints.md)). Webhook URL must be HTTPS. A shared secret (HMAC-SHA256) is included in the `X-TaxKlaro-Signature` header for verification.
...
3. Compare with `X-TaxKlaro-Signature` header (hex-encoded). If mismatch, reject as potential forgery.
...
A CPA adds clients to their roster by providing the client's registered TaxKlaro email address. The client must already have a TaxKlaro account (any tier — FREE, PRO, or ENTERPRISE).

**Invitation flow (client must consent):**

Step 1: CPA calls `POST /clients` with `client_email: "client@example.com"`.

Step 2: Server checks if a TaxKlaro user with that email exists. If yes, creates a `cpa_clients` row with `consent_status = 'PENDING'` and sends an email to the client:

**Email subject:** "A CPA has requested to manage your TaxKlaro account"

**Email body (exact):**
> "Hello,
>
> [CPA Full Name] from [Firm Name or 'independent practice'] has requested access to manage your TaxKlaro tax computations as your CPA.
>
...
> - Delete your TaxKlaro account
>
> To authorize: [Approve Access] button (links to https://app.taxklaro.ph/consent/{consent_token})
> To decline: [Decline] button (links to https://app.taxklaro.ph/consent/{consent_token}/decline)
>
...
**Bypass option for CPA-owned clients:** If the CPA's client does NOT have a TaxKlaro account, the CPA may create a "managed-only" client by calling `POST /clients` with `create_unregistered: true`. This creates a minimal client profile without an associated TaxKlaro user account. These clients cannot log in to view their own computations. Fields for unregistered clients:
...
- The client (if they have a TaxKlaro account) can also view this computation in their own history.
...
**Client account deletion (client deletes their TaxKlaro account):**
...
| ENTERPRISE | In-app help widget + dedicated email: enterprise@taxklaro.ph | 1 business day |
...
| Scheduled 30-minute call | Available for ENTERPRISE on request via enterprise@taxklaro.ph. Scheduling within 2 business days. Conducted via Google Meet. |
...
1. **Standard ticket:** Email to enterprise@taxklaro.ph or in-app widget. Assigned to support tier 1 within 1 BD.
...
3. **Critical filing deadline escalation:** Email to enterprise@taxklaro.ph with subject prefix "[URGENT]". Support monitors this inbox every 2 hours on business days. Response within 2 hours.
...
| PF-INV-16 | "Powered by TaxKlaro" must appear in the footer of all white-labeled PDF exports. This text cannot be removed by any configuration. | Hard-coded in PDF template |
