#!/bin/bash
# Reverse Ralph Loop — TaxKlaro Platform Completion
#
# Usage:
...
#
# This loop reads the existing domain spec from loops/freelance-tax-reverse/final-mega-spec/
# and adds missing platform/infrastructure layers to produce a unified mega-spec.
...
while [ "$iteration" -lt "$MAX_ITERATIONS" ]; do
    ...
    if [ "$pending" -eq 0 ]; then
        ...
        # Run one more time to let the agent write converged.txt
    fi
    ...
    iter_log="/tmp/freelance-tax-platform-reverse-iter-${iteration}.log"
