// WizardPage: Paginated wizard, one step at a time. Used for creating new computations.
// All 17 wizard step files are statically imported here (spec §14.2 rule 2).
...
// Static imports for all 17 wizard step files (spec §14.2 rule 2)
...
type StepComponent = React.ComponentType<{ data: Partial<WizardFormData>; onBack: () => void; onSubmit: () => void }>;
...
export function WizardPage({ initialData, onSubmit }: WizardPageProps) {
  const [data, setData] = useState<Partial<WizardFormData>>(initialData ?? {});
  const activeSteps = computeActiveSteps(data as WizardFormData);
  ...
  return (
    <div className="max-w-xl mx-auto py-8 px-4 space-y-6">
      <WizardProgressBar currentStep={stepIndex + 1} totalSteps={activeSteps.length} />
      {StepComponent ? (
        <StepComponent data={data} onBack={handleBack} onSubmit={handleNext} />
      ) : (
        <div>Unknown step: {currentStepId}</div>
      )}
      <WizardNavControls
        onBack={handleBack}
        onNext={handleNext}
